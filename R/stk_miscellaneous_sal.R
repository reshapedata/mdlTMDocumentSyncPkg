...
#'
#' @examples
#' TmWMS_stk_miscellaneous_sal_sekectBymonth()
TmWMS_stk_miscellaneous_sal_sekectBymonth<- function(wms_token,fyear,fmonth) {
  sql=paste0("select
'其他入库_工程不良'  as billtype ,
e.id as Fid ,
f.id as FEntryID ,
e.material_id ,
e.material_code ,
...
'03.01.05.01'  as  current_f_storck_id ,
f.product_order_id ,
...
#'
#' @examples
#' TmWMS_stk_miscellaneous_sal_deleteBymonth()
TmWMS_stk_miscellaneous_sal_deleteBymonth<- function(dms_token,fyear,fmonth) {
  sql=paste0("delete  from rds_dms_src_t_stk_miscellaneous_sal
...
#'
#' @examples
#' Tmdms_stk_miscellaneous_sal_selectByfbillno()
Tmdms_stk_miscellaneous_sal_selectByfbillno<- function(dms_token,fbillno) {
  sql=paste0("SELECT
...
FROM rds_dms_src_t_stk_miscellaneous_sal
where FID  = '",fbillno,"' ")
...
#'
#' @examples
#' Tmdms_stk_miscellaneous_sal_selectBydate()
Tmdms_stk_miscellaneous_sal_selectBydate<- function(dms_token,fdate) {
  sql=paste0("SELECT
FBilltypeName	as	单据类型	,
FID	as	主键ID	,
FentryID	as	表体ID	,
...
#'
#' @examples
#' Tmdms_stk_miscellaneous_sal_selectBydateRange()
Tmdms_stk_miscellaneous_sal_selectBydateRange<- function(dms_token,FStartDate,FEndDate) {
  sql=paste0("SELECT
FBilltypeName	as	单据类型	,
FID	as	主键ID	,
FentryID	as	表体ID	,
